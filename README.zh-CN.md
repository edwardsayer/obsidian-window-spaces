...
- **Popout 面板支持**：支持在 Popout 窗口内显示 Activity Bar 与左右侧边栏面板，不改变主窗口布局。
- **Pinned 优先智能命名**：自动优先提取置顶笔记 (`pinned: true`) 与焦点笔记生成直观名称。
- **情境感知还原 (Context-Aware)**：普通点击（或按 `Enter`）会在全新独立窗口中打开；按住 `Shift`（或 `Shift + Enter`）则应用至当前 active Popout 窗口。若某个 Space 已在 Popout 窗口中打开，再次还原时会直接聚焦该窗口，不会产生重复窗口。
- **Per-Layout 独立自动保存**：针对特定布局开启 🔄 自动保存，具备 5 秒 Debounce 静默更新与窗口关闭实时快照。
- **整合式快速切换与管理弹窗**：实时搜索、Hover 文件列表悬停预览、⚙️ 6 维度排序菜单、重命名与删除。可打开为弹出窗口（标题栏具备相同工具栏）、编辑器面板或侧边栏面板。
...
- 搜索并执行 `Window Spaces: 保存当前空间`。
- 使用默认智能名称或输入名称后按 `Enter`。

#### 2. 打开窗口布局
- 执行 `Window Spaces: 作为弹出窗口打开` 以弹出窗口打开，`作为标签页面板打开` 以标签页打开，或 `在左侧边栏打开` / `在右侧边栏打开` 停靠于侧边栏。标签页与侧边栏命令会在**当前聚焦的窗口**中打开：在弹窗中执行即可把面板开进该弹窗，在主窗口中执行则开进主工作区。
- 在列表中直接点击布局，或按 `Enter`，会在新的 Popout 窗口应用布局。
- 按住 `Shift` 点击布局，或按 `Shift + Enter`，会应用至当前 active Popout 窗口。
- 若某个 Space 已在 Popout 窗口中打开，再次还原时会直接聚焦该窗口，不会产生重复窗口。
- 通过下拉菜单 (∨) 切换自动保存、重命名、编辑或删除布局，并可用面板菜单移动选择器至其他位置。
...
### ⌨️ 键盘操作
- 插件没有预设指令快捷键；可在 Obsidian 快捷键设置中自行为 `作为弹出窗口打开`（或标签面板 / 侧边栏指令）指定快捷键。
- 列表中按 `Enter` 会在新 Popout 恢复，按 `Shift + Enter` 会在当前 active Popout 恢复。
- 方向键导航仅在面板自身获得焦点时生效；在命令面板、重命名对话框、下拉菜单或其它窗口中输入时不会被拦截。
