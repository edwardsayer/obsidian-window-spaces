...
- **Popout 面板支援**：支援在 Popout 視窗內呈現 Activity Bar 與左右側邊欄面板，不改變主視窗 layout。
...
#### 2. 開啟視窗佈局
- 執行 `Window Spaces: 開啟為彈出視窗` 以彈出視窗開啟，`開啟為標籤面板` 以分頁開啟，或 `開啟在左側邊欄` / `開啟在右側邊欄` 停靠於側邊欄。分頁與側邊欄指令會在**目前聚焦的視窗**中開啟：在彈出視窗執行即可把面板開進該彈出視窗，在主視窗執行則開進主工作區。
...
- 方向鍵導覽僅在面板本身取得焦點時生效；在命令面板、重新命名對話框、下拉選單或其他視窗中輸入時不會被攔截。
